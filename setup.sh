#!/usr/bin/env bash
# =============================================================================
# One-command setup for TwitterAuto
...
phase_env() {
  ...
  "TWITTER_API_KEY|Twitter/X API key|Twitter/X developer API key for read-only AI trend retrieval.|Open https://developer.x.com/en/portal/dashboard in your browser\nSign in and select the project and app used for TwitterAuto\nClick 'Keys and tokens' for the app\nFind the 'API Key and Secret' section\nCopy the API Key and paste it into the field above."
  "TWITTER_API_SECRET|Twitter/X API secret|Twitter/X developer API secret for read-only AI trend retrieval.|Open https://developer.x.com/en/portal/dashboard in your browser\nSign in and select the project and app used for TwitterAuto\nClick 'Keys and tokens' for the app\nFind the 'API Key and Secret' section\nCopy the API Secret and paste it into the field above."
  "TWITTER_ACCESS_TOKEN|Twitter/X access token|Twitter/X access token for read-only trend/source retrieval.|Open https://developer.x.com/en/portal/dashboard in your browser\nSign in and select the project and app used for TwitterAuto\nClick 'Keys and tokens' for the app\nFind the 'Access Token and Secret' section\nClick 'Generate' if a token is not already available\nCopy the Access Token and paste it into the field above."
  "TWITTER_ACCESS_TOKEN_SECRET|Twitter/X access token secret|Twitter/X access token secret for read-only trend/source retrieval.|Open https://developer.x.com/en/portal/dashboard in your browser\nSign in and select the project and app used for TwitterAuto\nClick 'Keys and tokens' for the app\nFind the 'Access Token and Secret' section\nClick 'Generate' if a token secret is not already available\nCopy the Access Token Secret and paste it into the field above."
  "RESEND_API_KEY|Resend API key|Resend API key for approval email delivery.|Open https://resend.com/api-keys in your browser\nSign in to the Resend workspace used for TwitterAuto\nClick 'Create API Key'\nEnter a name such as 'TwitterAuto approval email' and choose sending permissions\nClick 'Add' to create the key\nCopy the API key and paste it into the field above."
  "RECIPIENT_EMAIL_ADDRESS|Approval recipient email address|Email address where the requesting user receives drafts for approval.|Open https://mail.google.com or your preferred email inbox in your browser\nConfirm the inbox where the requesting user will review TwitterAuto draft emails\nCopy the recipient email address and paste it into the field above."
  "EMAIL_FROM_ADDRESS|Verified Resend sender email address|Verified sender address used by Resend.|Open https://resend.com/domains in your browser\nSign in to the Resend workspace used for TwitterAuto\nClick 'Domains' and verify a sending domain if needed\nOpen the verified domain and choose the sender address for approval emails\nCopy the verified sender address and paste it into the field above."
  )
...
phase_db() {
  ...
  if [[ -n "${PG_CONNECTION_STRING:-}" ]]; then
    ...
    if ${local_pg}; then
      ...
      warn "     Then set in .env: PG_CONNECTION_STRING=postgresql://postgres:<choose-a-password>@localhost:5432/twitterauto"
      exit 1
    fi
...
phase_finish() {
  ...
  if command -v openclaw >/dev/null 2>&1; then
    if openclaw cron list 2>/dev/null | grep -q "^manual-tweet-draft-request\b"; then
      ok "cron 'manual-tweet-draft-request' already registered"
    else
      openclaw cron add --file cron/manual-tweet-draft-request.json && ok "registered cron 'manual-tweet-draft-request'"
    fi
  else
    warn "openclaw CLI not found — register manually: openclaw cron add --file cron/manual-tweet-draft-request.json"
  fi
  ...
  log "  openclaw cron run --name manual-tweet-draft-request    # trigger manually"
